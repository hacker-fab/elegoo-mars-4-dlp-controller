...
class DLPC1438:
    ...
    def configure_external_print(self, LED_PWM, skip_FPGA_video=False):
        ...
        PWM_data = list(LED_PWM.to_bytes(2, byteorder="little")) + [
            0x00,
            0x00,
            0x00,
            0x00,
    ...
    def expose_pattern(self, exposed_frames, dark_frames=5):
        ...
        if exposed_frames > 0:
            ...
            frame_data = [0x00] + list(dark_frames.to_bytes(2, byteorder="little")) + list(exposed_frames.to_bytes(2, byteorder="little"))
            ...
            self.__i2c_write(
                0xC1,
                [0x00] + list(dark_frames.to_bytes(2, byteorder="little")) + [0xFF, 0xFF],
            )
            # the value above never seem to be read back correctly if you read over I2C. Also on anyubic board.'
        ...
        if not GPIO.input(self.PRINT_ACTIVE):
            warnings.warn("PRINT ACTIVE did not go high after starting exposure. Something might be wrong.")
    ...
    def stop_exposure(self):
        ...
        if GPIO.input(self.PRINT_ACTIVE):
            warnings.warn("PRINT ACTIVE is still high after sending STOP uv expose command. Something might be wrong")
    ...
    def split_spi_transmission(self, offset_width, offset_height, pixel_data):
        ...
        padded_data = np.pad(
            pixel_data,
            ((pad_width_start, pad_width_end), (pad_height_start, pad_height_end)),
        )
    ...
    def test_FPGA(self):
        ...
        if not GPIO.input(self.SPI_RDY):
            warnings.warn("FPGA not ready to receive SPI data")
